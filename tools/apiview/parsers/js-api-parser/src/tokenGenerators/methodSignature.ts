import { ApiMethodSignature, ApiItem, ApiItemKind } from "@microsoft/api-extractor-model";
import { ReviewToken, TokenKind } from "../models";
import { buildToken } from "../jstokens";
import { TokenGenerator } from "./interfaces";

function isValidFor(item: ApiItem): item is ApiMethodSignature {
  return item.kind === ApiItemKind.MethodSignature;
}

function generate(item: ApiMethodSignature): ReviewToken[] {
  const tokens: ReviewToken[] = [];
  
  if (item.kind !== ApiItemKind.MethodSignature) {
    throw new Error(
      `Invalid item ${item.displayName} of kind ${item.kind} passed to method signature generator`,
    );
  }
  ...
  // Optional marker (for optional method signatures like: methodName?(): void)
  if (item.isOptional) {
  ...
  if (item.parameters && item.parameters.length > 0) {
    item.parameters.forEach((param, i) => {
      ...
      // Optional marker (for optional parameters)
      if (param.isOptional) {
        tokens.push(buildToken({ Kind: TokenKind.Punctuation, Value: "?" }));
      }
...
export const methodSignatureTokenGenerator: TokenGenerator = {
  isValidFor,
  generate,
};
